...
setColorBG :: Color -> String -> String
setColorBG c s = "<bc=" ++ c ++ ">" ++ s ++ "</bc>"
...
config = defaultConfig
  ...
  , sepChar = "$"
  , alignSep = "}{"
  , template = "$cpu$ | $memory$ * $swap$ | $eth0$} {" ++ setColorBG blue (setColorFG yellow "$date$")
  }
